...
func EstablishP2P() *P2P {
	...
	// Debug log
	fmt.Fprintln(File, "Created the P2P Host and the Kademlia DHT.")

	// Bootstrap the Kad DHT
	bootstrapDHT(ctx, nodehost, kaddht)
	// Debug log
	fmt.Fprintln(File, "Bootstrapped the Kademlia DHT and Connected to Bootstrap Peers")
	// Create a peer discovery service using the Kad DHT
	routingdiscovery := discovery.NewRoutingDiscovery(kaddht)
	// Debug log
	fmt.Fprintln(File, "Created the Peer Discovery Service.")

	// Create a PubSub handler with the routing discovery
	pubsubhandler := setupPubSub(ctx, nodehost, routingdiscovery)
	// Debug log
	fmt.Fprintln(File, "Created the PubSub Handler.")
...
// 	fmt.Println("Using announcement")
...
func setupHost(ctx context.Context) (host.Host, *dht.IpfsDHT) {
	...
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "Failed to Generate P2P Identity Configuration!")
	}
	...
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "Failed to Generate P2P Security and Transport Configurations!")
	}

	// Trace log
	fmt.Fprintln(File, "Generated P2P Security and Transport Configurations.")
	...
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "Failed to Generate P2P Address Listener Configuration!")
	}

	// Trace log
	fmt.Fprintln(File, "Generated P2P Address Listener Configuration.")
	...
	// Trace log
	fmt.Fprintln(File, "Generated P2P Stream Multiplexer, Connection Manager Configurations.")
	...
	// Trace log
	fmt.Fprintln(File, "Generated P2P NAT Traversal and Relay Configurations.")
	...
	// Trace log
	fmt.Fprintln(File, "Generated P2P Routing Configurations.")
	...
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "Failed to Create the P2P Host!")
	}
...
func setupKadDHT(ctx context.Context, nodehost host.Host) *dht.IpfsDHT {
	...
	// Trace log
	fmt.Fprintln(File, "Generated DHT Configuration.")

	// Start a Kademlia DHT on the host in server mode
	kaddht, err := dht.New(ctx, nodehost, dhtmode, dhtpeers)
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "Failed to Create the Kademlia DHT!")
	}
...
func setupPubSub(ctx context.Context, nodehost host.Host, routingdiscovery *discovery.RoutingDiscovery) *pubsub.PubSub {
	...
	// Handle any potential error
	if err != nil {
		fmt.Fprintln(File, "PubSub Handler Creation Failed!")
	}
...
func bootstrapDHT(ctx context.Context, nodehost host.Host, kaddht *dht.IpfsDHT) {
	...
	if err := kaddht.Bootstrap(ctx); err != nil {
		fmt.Fprintln(File, "Failed to Bootstrap the Kademlia!")
	}
	...
	fmt.Fprintln(File, "Connected to %d out of %d Bootstrap Peers.", connectedbootpeers, totalbootpeers)
}
...
func handlePeerDiscovery(nodehost host.Host, peerchan <-chan peer.AddrInfo) {
	...
	for peer := range peerchan {
		...
		if err != nil {
			fmt.Fprintln(File, "connection to ", peer.Addrs, "failed")
		} else {
			fmt.Fprintln(File, "connection to ", peer.Addrs, " successfull")
		}
	}
	fmt.Fprintln(File, "Connection to service nodes are done")
}
...
func generateCID(namestring string) cid.Cid {
	...
	if err != nil {
		fmt.Fprintln(File, "Failed to Generate Service CID!")
	}
